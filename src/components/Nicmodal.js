...
const  Nicmodal = ({ closeModal, leftImage, rightImage }) => {
  const [currentImage, setCurrentImage] = useState(leftImage);
  const iconStyle = {
    width: '3rem',
    height: '3rem',
    stroke: 'white',
      ...
      <div className="relative">
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 w-screen h-screen" style={modalStyle}>
          <div className="rounded-xl flex items-center w-auto p-4">
            ...
            <div className="relative">
              <img src={currentImage} className="object-cover" style={imageStyle} alt='nic'/>
            </div>
